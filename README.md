...
**DTMerger** は、**DocuWorks 文書（.xdw）**、**PDF（.pdf）**、**TIFF 画像（.tif / .tiff）** をひとつにまとめ、**マルチページ TIFF** として出力するアプリです。

- DocuWorks 文書とPDFは、画面で指定したDPIの画像に変換したうえで結合します。
...
1. エクスプローラーから、結合したい `*.xdw` / `*.pdf` / `*.tif` / `*.tiff` を選びます。
...
### 6.1 DocuWorks 文書の変換
- DocuWorks 文書（.xdw）は、各ページを画面の「DW変換DPI」で指定した解像度のTIFF画像に変換してから結合します。

### 6.2 PDFの変換
- PDF（.pdf）は、各ページを画面の「DW変換DPI」で指定した解像度の画像に変換してから結合します。
...
- DocuWorks 文書とPDFの変換解像度は、画面の「DW変換DPI」で指定します。
